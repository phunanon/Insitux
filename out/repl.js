...
const val2str = ({ v, t }) => {
  const quoted = (v2) => v2.t === "str" ? `"${v2.v}"` : val2str(v2);
  switch (t) {
    case "bool":
      return `${v}`;
    case "num":
...
const isDictEqual = (a, b) => {
  ...
  return src_len(ad.keys) === src_len(bd.keys) && isVecEqual(ad.keys, bd.keys);
};
const isEqual = (a, b) => {
  const { t } = a;
  switch (t) {
...
async function exeOp(op, args, ctx, errCtx, checkArity) {
  ...
  switch (op) {
    ...
    case "len":
      _num(args[0].t === "str" ? src_slen(str(args[0])) : args[0].t === "vec" ? src_len(vec(args[0])) : src_len(dic(args[0]).keys));
      return;
    case "to-num":
    ...
    case "idx": {
      ...
      if (args[0].t === "str") {
        ...
        if (src_len(args) < 3) {
          i = src_subIdx(str(args[0]), str(args[1]));
        } else {
    ...
    case "into": {
      ...
      if (a0v) {
        _vec(src_concat(vec(args[0]), a1v ? vec(args[1]) : asArray(args[1])));
      } else {
        if (a1v) {
          const v1 = asArray(args[1]);
          stack.push(toDict(src_concat(src_flat(asArray(args[0]).map(vec)), v1)));
        } else {
    ...
    case "sect": {
      const v = args[0];
      const isVec = v.t === "vec";
      const vlen = isVec ? src_len(vec(v)) : src_slen(str(v));
      let a = 0, b = vlen;
      switch (src_len(args)) {
        case 1:
          a = 1;
    ...
    case "join":
      ...
      return;
    case "starts-with?":
      _boo(src_starts(str(args[0]), str(args[1])));
      return;
    case "ends-with?":
      _boo(src_ends(str(args[0]), str(args[1])));
      return;
    case "upper-case":
    case "lower-case":
    case "trim":
    ...
    case "tests":
      {
        const tests = await doTests(invoke, !(src_len(args) && asBoo(args[0])));
...
function getExe(ctx, op, errCtx, checkArity = true) {
  ...
  if (visStr(op) || visFun(op)) {
    const str2 = op.v;
    if (ops[str2]) {
      return (params) => exeOp(str2, params, ctx, errCtx, checkArity);
    }
    if (str2 in ctx.env.funcs) {
      return (params) => exeFunc(ctx, ctx.env.funcs[str2], params);
    }
    if (str2 in ctx.env.vars) {
      return getExe(ctx, ctx.env.vars[str2], errCtx);
    }
    if (str2 in ctx.env.lets[src_len(ctx.env.lets) - 1]) {
      return getExe(ctx, ctx.env.lets[src_len(ctx.env.lets) - 1][str2], errCtx);
    }
    if (src_starts(str2, "$")) {
      return async (params) => {
        if (!src_len(params)) {
        ...
        const err = await ctx.set(src_substr(str2, 1), params[0]);
        stack.push(params[0]);
        return err ? [{ e: "External", m: err, errCtx }] : void 0;
      };
    }
    return async (params) => {
      const { err, value } = await ctx.exe(str2, params);
      if (!err) {
        stack.push(value);
      }
      return err ? [{ e: "External", m: err, errCtx }] : void 0;
