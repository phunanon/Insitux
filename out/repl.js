...
function arityCheck(op, nArg, errCtx) {
  const { exactArity, maxArity, minArity } = types_ops[op];
...
function typeCheck(op, args, errCtx, optimistic = false) {
  const { params: types, numeric: onlyNum } = types_ops[op];
  const nArg = len(args);
  if (onlyNum) {
...
function parseForm(nodes, params, doArityCheck = true) {
  ...
  if (isToken(firstNode) && firstNode.typ === "sym") {
    if (firstNode.text in types_ops) {
      const { exactArity, minArity } = types_ops[firstNode.text];
    ...
    if (types_ops[op] && doArityCheck) {
      ...
      if (!errors) {
        if (parse_len(nodes) === 2 && types_ops[`fast${op}`]) {
    ...
    switch (ins.typ) {
      ...
      case "exe": {
        ...
        if (head.val && head.val.t === "func") {
          ...
          const { returns, numeric: onlyNum } = types_ops[head.val.v];
...
function ingestExternalOperations(functions) {
  Object.keys(functions).forEach((name) => {
    if (types_ops[name] && !types_ops[name].external) {
      throw "Redefining internal operations is disallowed.";
    }
    types_ops[name] = { ...functions[name].definition, external: true };
  });
}
function removeExternalOperations(functionNames) {
...
function symbols(env, alsoSyntax = true) {
  ...
  syms = src_concat(syms, src_objKeys(types_ops));
  ...
  return {
    ...
    import: {
      ...
      handler: (params) => {
        ...
        const path = (0,external_path_namespaceObject.join)(workingDirectory, isGithub ? `.ix/${p0}/entry.ix` : isAliased ? `.ix/${p0}.ix` : p0);
        if (!(0,external_fs_.existsSync)(path)) {
          return { kind: "err", err: `not found: ${path}` };
        }
        const code = (0,external_fs_.readFileSync)(path).toString();
        const oldFuncs = ctx.functions;
        ctx.functions = makeFunctions((0,external_path_namespaceObject.dirname)(path));
...
async function dependencyResolve(dependency) {
  ...
  try {
    ...
    } else if (dependency.kind === "Github install" || dependency.kind === "Github remove") {
      const path = `.ix/${dependency.repo}`;
      if ((0,external_fs_.existsSync)(path)) {
        (0,external_fs_.rmSync)(`.ix/${dependency.repo.split("/")[0]}`, {
          recursive: true,
          force: true
        });
      }
...
async function depsFileAction(action) {
  const path = "deps.txt";
  if (!(0,external_fs_.existsSync)(path)) {
    console.log(`${path} not found.`);
    return;
  }
  const deps = (0,external_fs_.readFileSync)(path).toString().split("\n").map((x) => x.trim()).filter((x) => x);
  if (!deps.length) {
...
async function processCliArguments(args) {
  if (!args.length) {
    startRepl();
    return;
  }
