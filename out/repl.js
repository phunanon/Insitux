...
const nodeFetch = __webpack_require__(85)
...
const spawn = __webpack_require__(81).spawn;
...
const impl = __webpack_require__(531);
...
async function exeOp(op, args, ctx, errCtx) {
  switch (op) {
    ...
    case "count": {
      ...
      if (op === "for") {
        ...
        for (let t = 0; t < lim; ++t) {
          ...
          array2.push(await closure(arrays.map((a2, i) => a2[argIdxs[i]])));
        }
        return _vec(array2);
      }
      if (op === "map") {
        ...
        for (let i = 0; i < shortest; ++i) {
          array2.push(await closure(arrays.map((a2) => a2[i])));
        }
        return _vec(array2);
      }
      if (op !== "reduce") {
        ...
        for (let i = 0, lim = src_len(array2); i < lim; ++i) {
          const b2 = asBoo(await closure([array2[i], ...args]));
          if (isCount) {
            count += b2 ? 1 : 0;
          } else if (isFind) {
            if (b2) {
    ...
    case "tests": {
      const summaries = await doTests(invoke, !(src_len(args) && asBoo(args[0])));
...
function getExe(ctx, op, errCtx, checkArity = true) {
  if (op.t === "str" || op.t === "func") {
    ...
    if (name in ctx.env.funcs && name !== "entry") {
      return async (params) => await exeFunc(ctx, ctx.env.funcs[name], params);
    }
...
async function exeFunc(ctx, func, args, closureDeref = false) {
  --ctx.callBudget;
  if (!closureDeref) {
    letsStack.push({});
  ...
  for (let i = 0, lim = src_len(func.ins); i < lim; ++i) {
    ...
    switch (ins.typ) {
      ...
      case "clo": {
        ...
        const captures = (await exeFunc(ctx, { ins: derefIns }, args, true)).v;
...
async function parseAndExe(ctx, code, invokeId, params) {
...
async function innerInvoke(ctx, closure, printResult) {
...
async function invoke(ctx, code, invokeId, printResult = false, params = []) {
  return await innerInvoke(ctx, async () => await parseAndExe(ctx, code, invokeId, params), printResult);
}
async function src_invokeFunction(ctx, funcName, params, printResult = false) {
  if (!(funcName in ctx.env.funcs)) {
    return;
  }
  return await innerInvoke(ctx, async () => await exeFunc(ctx, ctx.env.funcs[funcName], params), printResult);
}
function symbols(env, alsoSyntax = true) {
  let syms = [];
  if (alsoSyntax) {
  ...
  syms = src_concat(syms, src_objKeys(env.funcs));
  syms = src_concat(syms, src_objKeys(env.vars));
  const hidden = ["entry"];
...
const parensRx = /[\[\]\(\) ,]/;
async function invoker(ctx, code, id, printResult = true) {
  id = id ? `-${id}` : `${getTimeMs()}`;
  invocations.set(id, code);
  const result = await invoke(ctx, code, id, printResult);
  return { output: invokeResultToOutput(result), result };
}
async function functionInvoker(ctx, name, params, printResult = true) {
  const result = await invokeFunction(ctx, name, params, printResult);
  if (!result) {
    const message = {
      type: "message",
...
function invokeResultToOutput(result) {
...
const functions = {
  ...
  "file-append": {
    definition: writingOpDef,
    handler: async (params) => writeOrAppend(params[0].v, params[1].v, true)
  },
  prompt: {
    definition: {
      exactArity: 1,
  ...
  import: {
    ...
    handler: async (params, ctx2) => {
      const p0 = str(params[0]);
      const isGithub = params.length === 1 && githubRegex.test(p0);
      const isAliased = params.length === 2;
      const workingDir = (0,external_path_namespaceObject.join)(ctx2.innerCtx.workingDirectory, isGithub ? `./ix/${p0}` : isAliased ? ".ix" : "");
      const fileName = isGithub ? "entry.ix" : p0;
      const path = (0,external_path_namespaceObject.join)(workingDir, fileName);
      if (!(0,external_fs_.existsSync)(path)) {
        if (isGithub) {
      ...
      const { result, output } = await invoker(ctx2, code, path, false);
      if (result.kind === "errors") {
...
const ctx = {
  ...
  set: repl_set,
  functions,
  print(str2, withNewLine) {
    process.stdout.write(`[32m${str2}[0m${withNewLine ? "\n" : ""}`);
  },
  exe: repl_exe
};
...
async function dependencyResolve(dependency) {
  (0,external_fs_.mkdirSync)(".ix", { recursive: true });
  try {
    switch (dependency.kind) {
      case "http install": {
        const response = await node_ponyfill_default()(dependency.url);
        if (!response.ok) {
          throw `${response.status}: ${response.statusText}: ${dependency.url}`;
        }
        const text = await response.text();
        (0,external_fs_.writeFileSync)(`.ix/${dependency.alias}.ix`, text);
        break;
      }
      case "alias remove":
        (0,external_fs_.unlinkSync)(`.ix/${dependency.alias}.ix`);
        break;
      case "Github install": {
        const path = `.ix/${dependency.repo}`;
        if ((0,external_fs_.existsSync)(path)) {
          (0,external_fs_.unlinkSync)(`.ix/${dependency.repo}`);
        }
...
async function startCli([nodePath, workingDir, ...args]) {
  if (!args.length) {
    startRepl();
    return;
  }
...
async function startRepl() {
  ...
  if ((0,external_fs_.existsSync)(".repl.ix")) {
    printErrorOutput(await invoker(ctx, (0,external_fs_.readFileSync)(".repl.ix").toString()));
  }
...
let lines = [];
function haveFinishedEntry(code) {
